...
from tensorflow.keras.layers import LSTM, Embedding, Dense,Conv1D, MaxPooling1D, GRU,Bidirectional
...
class GapPredictModel:
    def _initialize_models(self):
        ...
        if self.stateful:
            ...
            model.add(Conv1D(filters=self.cnn_filter, kernel_size=3, activation='relu',
                             input_shape=(None, self.embedding_dim)))

            model.add(Bidirectional(LSTM(self.latent_dim, stateful=self.stateful, input_shape=(None, self.embedding_dim))))
    ...
    def __init__(self, min_seed_length, spacing=0, batch_size=64, stateful=False, epochs=100, embedding_dim=25,
                 latent_dim=100, cnn_filter=32, with_gpu=True, log_samples=True, reference_sequences=None,log_training=False,
