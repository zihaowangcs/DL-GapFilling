...
from predict.WaveBeamSearchPredictor import WaveBeamSearchPredictor
...
def predict_arbitrary_length(weights_path, gap_id, fasta_path,batch_size ,embedding_dim, latent_dim,cnn_filter, length_to_predict, beam_length, base_path=None):
    ...
    model = GapPredictModel(min_seed_length=None, stateful=False, batch_size=batch_size, embedding_dim=embedding_dim,
                            latent_dim=latent_dim, cnn_filter=cnn_filter, with_gpu=True)
...
def predict(model, seed, prediction_length, beam_length, flank_id, base_path=None, directory=None):
    predictor = WaveBeamSearchPredictor(model)
