...
def parse_gaps_flanks_idx(id):
    id_copy = str(id)
    id_copy = strip_terminal_newline(id_copy)
    id = id_copy.split(":")[1].split("-")[0]
    return id

def main():
	ids = ["fixed", "unfixed"]
	for file_id in ids:
		fasta_file = open(sys.argv[1] + '/gene-scaffolds_' + file_id + '_gaps_flanks_random_sample.fa', 'r')
		reads_file = open(sys.argv[1] + '/gene-scaffolds_' + file_id + '_gaps_flanks_random_sample.fastq', 'r')
		predict_file = open(sys.argv[1] + '/gene-scaffolds_' + file_id + '_gaps_flanks_with_both.fa', 'r')
		...
		gaps_flanks_map = {}
		if file_id is "fixed":
			sequence = next_n_lines(predict_file, 2)
			while sequence is not None:
				id = parse_gaps_flanks_id(sequence[0])
				left_flank_right_endpoint = parse_gaps_flanks_idx(sequence[0])
				map_id = id + "-" + left_flank_right_endpoint
				if map_id not in gaps_flanks_map:
					gaps_flanks_map[map_id] = sequence
				else:
					print("Error")
					return
				sequence = next_n_lines(predict_file, 2)
		...
		while pair is not None:
			...
			gap_flank_fasta_output=open(new_folder + full_id + "_gap_flank.fasta","w+")
			...
			if file_id is "fixed":
				...
				gaps_flanks_map_id = contig_id + "-" + str(left_flank_range[0])
				if map_id not in gaps_map:
					print("Error")
					return
				else:
					...
					if gaps_flanks_map_id not in gaps_flanks_map:
						print("Error",gaps_flanks_map_id +"\n"+sequence[1])
						shutil.rmtree(new_folder)
					else:
						sequence = gaps_flanks_map[gaps_flanks_map_id]
						write_all_to_dest(gap_flank_fasta_output, sequence)
