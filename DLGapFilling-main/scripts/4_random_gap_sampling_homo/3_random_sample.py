...
def write_bed_to_dest(dest_file,buf,is_last):
	start_index = -1
	end_index = -1
	for i in range(len(buf)):
		if i % 2 == 0:
			full_id = strip_terminal_newline(str(buf[i]).split(">")[1])
			id_str = full_id.split(":")[0]
			start_index = int(full_id.split(":")[1].split("-")[0]) if start_index == -1 else (start_index if int(full_id.split(":")[1].split("-")[0])> start_index else int(full_id.split(":")[1].split("-")[0]))
			end_index = int(full_id.split(":")[1].split("-")[1]) if end_index == -1 else (end_index if int(full_id.split(":")[1].split("-")[1])< end_index else int(full_id.split(":")[1].split("-")[1]))

			lines = str(id_str)+"\t"+str(start_index)+"\t"+str(end_index) if is_last else str(id_str)+"\t"+str(start_index)+"\t"+str(end_index)+"\n"
...
def main():
	ids = ["fixed", "unfixed"]
	for file_id in ids:
		gaps_file = open(sys.argv[1] + '/gene-scaffolds_' + file_id + '_gaps_flanks.fa', 'r')
		output_file = open(sys.argv[1] + '/gene-scaffolds_' + file_id + '_gaps_flanks_random_sample.fa', 'w+')
		id_file = open(sys.argv[1] +  '/gene-scaffolds_' + file_id + '_gaps_flanks_random_sample_ids.txt', 'w+')
		bed_gap_flanks_file = open(sys.argv[1] + "/gene-scaffolds_" + file_id + "_gaps_flanks_with_both.bed","w+")
		...
		for i in range(len(random_sample)):
			...
			write_bed_to_dest(bed_gap_flanks_file,pair,is_last)

		gaps_file.close()
		output_file.close()
		id_file.close()
		bed_gap_flanks_file.close()
